...
public:
	BatAlgorithm(std::function<double(double, double)> f, const glm::vec2& v1, const glm::bvec2 v2, int N = 50, double alpha = 0.9, double gamma = 0.9, double fmin = 0, double fmax = 1)
	{
		this->f = f;
		this->v1 = v1;
		this->v2 = v2;
...
private:
	...
	{
		//bats[i].f = fmin + (fmax - fmin)*beta(engine);
		bats[i].veliocity += ((bats[i].position - bestSoluion)*glm::vec2(bats[i].f));
		if (i == 0)
